...
export async function botDeleteAll(ctx: MyContext) {
  ...
  await ctx.reply(
    "⚠️ *ВНИМАНИЕ!*\n\n" +
    "Это удалит:\n" +
    `• ${stats.catalogsCount} каталог${stats.catalogsCount > 1 ? "ов" : ""}\n` +
...
export async function confirmDeleteAll(ctx: MyContext) {
...
export async function cancelDelete(ctx: MyContext) {
